...
rg -Fq 'Backend-driven effective access is active' "$route"
rg -Fq 'Effective access requires visibility and backend permission' "$route"
...
rg -Fq 'Visible + allowed' "$route"
rg -Fq 'Visible but blocked by missing permission' "$route"
rg -Fq 'Hidden but permission exists' "$route"
rg -Fq 'Missing backend contract' "$route"
rg -Fq 'Effective-access preview' "$route"
...
rg -Fq '## Milestone 89 - Roles and Permissions Effective-access UX' "$status"
