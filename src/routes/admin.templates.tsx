...
function TemplatesAdminPage() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: (row) => { toast.success(`Created "${row.title}"`); invalidate(); setDrawerOpen(false); },
  ...
  const updateMut = useMutation({
    ...
    onSuccess: () => { toast.success("Template updated"); invalidate(); setDrawerOpen(false); },
  ...
  const openEdit = (r: TicketCannedResponse) => { setEditId(r.id); setForm(fromRow(r)); setDrawerOpen(true); };
    ...
    <div>
      ...
      <div className="glass-card mb-4 rounded-2xl p-4">
        <div className="grid gap-3 md:grid-cols-12">
          <div className="md:col-span-6">
            <SearchInput value={query} onChange={setQuery} placeholder="Search shortcut, title, body…" />
          </div>
          <div className="md:col-span-3 flex items-center gap-2">
            <Button size="sm" variant={scopeFilter === "all" ? "default" : "ghost"} onClick={() => setScopeFilter("all")}>All</Button>
            <Button size="sm" variant={scopeFilter === "public" ? "default" : "ghost"} onClick={() => setScopeFilter("public")}>Public</Button>
            <Button size="sm" variant={scopeFilter === "internal" ? "default" : "ghost"} onClick={() => setScopeFilter("internal")}>Internal</Button>
          </div>
      ...
      <div className="glass-card overflow-hidden rounded-2xl">
        {isLoading ? (
          ...
          <EmptyState icon={Search} title="Failed to load templates" description={error instanceof Error ? error.message : "Unknown error"} />
        ) : filtered.length === 0 ? (
          <EmptyState
            icon={Search}
            title="No templates"
            description={writable ? "Create your first canned response." : "Ask an admin to add reply templates."}
          />
        ) : (
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {filtered.map((r) => (
                  <TableRow key={r.id}>
                    <TableCell className="font-mono text-xs">/{r.shortcut}</TableCell>
                    <TableCell className="font-medium">
                      {r.title}
                      <div className="truncate text-[11px] text-muted-foreground max-w-[480px]">{r.body}</div>
                    </TableCell>
                    <TableCell>
                      <StatusBadge tone={r.isInternal ? "warning" : "info"} label={r.isInternal ? "internal" : "public"} />
                    </TableCell>
      ...
      >
        <Field label="Shortcut">
          ...
          />
          <p className="mt-1 text-[11px] text-muted-foreground">Used as <code>/{form.shortcut || "shortcut"}</code> in the reply composer.</p>
        </Field>
        <Field label="Title">
        ...
        <div className="flex items-center justify-between rounded-md border border-border/50 bg-card/30 px-3 py-2">
          <div>
          ...
          <Switch checked={form.isInternal} onCheckedChange={(v) => setForm({ ...form, isInternal: v })} />
        </div>
      </FormDrawer>
