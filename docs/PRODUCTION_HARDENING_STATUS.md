...
## Milestone 83 - DB-backed Page Visibility Enforcement
...
Implementation:
- Added one shared authenticated read and decision layer for page visibility.
- The live matrix is usable only when it has exactly one platform-scoped row
  for every known static route pattern and mapped frontend role.
- Frontend `super_admin` maps to DB `platform_admin`, frontend `auditor` maps
  to DB `platform_auditor`, and all other known roles map directly.
- Dynamic paths resolve only through explicit static patterns such as
  `/tickets/:id`, `/service-catalog/:id`, and `/protocols/:id`. Unknown paths
  remain denied by the page-visibility helper.
- Matrix validation rejects missing, duplicate, extra, non-platform, or
  unknown cells. It also requires Platform Administrator access to
  `/admin/roles` and Employee denial for every known `/admin/*` route.
- `AuthGate` and `AppSidebar` use the live matrix when it passes validation.
  Loading, read errors, invalid matrices, and unrecognised account roles use
  the existing static `PAGE_VISIBILITY` rules instead.
- Existing authentication, identity-context loading, unknown-admin-route,
  redirect, and server-mediated edit behavior remains intact.
- The `/admin/roles` notice now describes live routing with static fallback.
...
Next gate:
- Perform production browser validation with manual platform administrator,
  employee, auditor, and operational-role route/sidebar checks.
